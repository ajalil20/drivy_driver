...
class Utils {
  ...
  static bool uniqueTimeCheck({required List<TimeOfDay> sTime,required List<TimeOfDay> eTime,required int startTime,required int endTime,}){
    for(int i=0;i<sTime.length;i++){
      if(Utils().convertTimeToMinutes(h: sTime[i].hour, m: sTime[i].minute)> Utils().convertTimeToMinutes(h: startTime, m: startTime)){
       return true;
      }
    }
    return false;
  ...
  static String convertToLocalTime({required String time,String? format}) {
    ...
    DateTime temp2 = DateFormat("yyyy-MM-dd hh:mm").parse('${year}-${month}-${date} ${hour}:${min}Z');
  ...
  selectDate(BuildContext context,{DateTime? firstDate,DateTime? lastDate, initialDate}) async {
  ...
  getUserChat({required User? u1,required User? u2,required User currentUser}){
  ...
  goToNotificationRoute({required context,required NotificationModel n,required bool isPush}) async{
    ...
    if (n.notificationType==AppStrings.MSG_NOTIFICATION) {
      ...
      AppNavigation.navigateTo(context, AppRouteName.CHAT_SCREEN_ROUTE,arguments: ScreenArguments(u:User(id: n.senderId,fullName: n.senderName)));
    }
    if (n.notificationType==AppStrings.STREAM_STARTED_NOTIFICATION) {
      ...
      await HomeController.i.joinStreaming(context, onSuccess: (){AppNavigation.navigateTo(context, AppRouteName.LIVE_STREAM_ROUTE);}, roomId: n.roomId);
    }
    else if (n.notificationType==AppStrings.STREAM_REQUEST_NOTIFICATION ||n.notificationType==AppStrings.STREAM_REQUEST_CANCELLED_NOTIFICATION || n.notificationType==AppStrings.STREAM_NOTIFICATION || n.notificationType==AppStrings.ADMIN_NOTIFICATION) {
      if(isPush){
        HomeController h= Get.put(HomeController());
      }
      AppNavigation.navigateToRemovingAll(context, AppRouteName.HOME_SCREEN_ROUTE,);
   ...
   // }
      ...
      AppNavigation.navigateToRemovingAll(context, AppRouteName.HOME_SCREEN_ROUTE,arguments: ScreenArguments(index: 2));
    }
    else if (n.notificationType==AppStrings.CONTRACT_REQUEST_ACCEPTED_NOTIFICATION) {
      if(isPush){
        HomeController h= Get.put(HomeController());
      }
      ...
      if(isPush){
        HomeController h= Get.put(HomeController());
        AppNavigation.navigateTo(context, AppRouteName.HOME_SCREEN_ROUTE,arguments: ScreenArguments(index: 2));
      }
      await HomeController.i.getOrderDetails(context: context,o:OrderModel(id: n.orderId));
      ...
      await HomeController.i.getProductDetail(context,p: ProductsModel(id: n.productId));
...
// }
  ...
  String convertFormattedDateTime(String formattedDateTime, String outputFormatt) {
  ...
  static ScheduleModel createScheduleJson({required String title,required String description,required DateTime start,required DateTime end,required String startTime,required String endTime}){
    var s = Utils.convertTimeToUTC(date: start, time: startTime);
    var e = Utils.convertTimeToUTC(date: end, time: endTime);
    return ScheduleModel(id:'${DateTime.now()}',title:title,description:description,startTime: s,endTime: e );

    // return({'"id"':'"${DateTime.now()}"','"title"':'"$title"', '"description"':'"$description"','"start_time"':'"$s"', '"end_time"':'"$e"'});
}
  static String convertTimeToUTC({required DateTime date,required String time,String? dateFormat}) {
    DateTime tempDate = DateFormat("yyyy-MM-dd hh:mm a").parse('${date.year}-${date.month}-${date.day} $time');
